...
function get_country_params(country::String)
    ...
    return (
        ...
        Π_cat  = 0.0571,     # placeholder premium on CAT bonds
        share = 0.0
        )
end
...
function default_iteration_1P_RA!(; sigma_ey, rho_y, beta, wc_par_asymm, delta, mu_r,
    ...
    while diff_q > tol_q && iter_q < maxiter_q
        ...
        # Compute risk-adjusted expectation for repayment: loop over current state and debt grid
        e_defrel_adj = zeros(N_x, N_b_g)
        for i in 1:N_x
            for k in 1:N_b_g
                term = ((1 .- def_only_pf[:, k]) .* (1 .- rel_only_pf[:, k]) .* (1 .+ (1 - delta) .* q_g_pf_zeros[:, k]) .+
                        (1 .- def_only_pf[:, k]) .* rel_only_pf[:, k] .* (1 .+ (1 - delta) .* q_g_rel_pf[:, k]))
                e_defrel_adj[i, k] = dot(P_x[i, :] .* M_mat[i, :], term)
            end
        end
        
        # Optional: apply penalty if default probability is very high
        d_threshold = 0.9
        theta = 100.0
        penalty_new = ones(N_x, N_b_g)
        ...
        q_g_new = repeat(qrf_vec, 1, N_b_g) .* (penalty_new .* e_defrel_adj)
...
function main_country_1P_RA(country::String)
    N_y = 7
    N_h = 3
    N_x = N_y * N_h
    N_b_g = 50
    ...
    beta          = country_params.beta_RA
    wc_par_asymm  = country_params.wc_par_asymm_RA
    ...
    @save "output/sim_1P_PC_RA.jld2" meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim meanspread_hurr_sim medianspread_hurr_sim gdp_g_h_sim spread_g_h_sim b_g_mean i_x_sim cons_sim spread_sim def_freq_sim hur_freq_sim gdp_mean_store V_g_mean
...
@time begin
    result_1P_RA = main_country_1P_RA("Jamaica")
end

# Call the plotting function using the outputs from main_country:
plt = plot_bond_price_schedule_with_gdp(result_1P_RA.b_g_vec, result_1P_RA.q_g, result_1P_RA.gdp_vec, result_1P_RA.meanDebtGDP)
display(plt)

# Saving model simulations results
V_g_1P_RA = result_1P_RA.V_g_mean
gamma_c = result_1P_RA.gamma_c

@save "output/Vg_sim_1P_RA.jld2" V_g_1P_RA gamma_c