...
function default_iteration_2P_RN!(; sigma_ey, rho_y, beta, wc_par_asymm, delta, mu_r,
    ...
    #########################################################################
    # 1. Debt Grid and State-Space Setup with Relief Flag
    #########################################################################
    ...
    # 2. Debt Relief Setup
    #########################################################################
    # First-period relief: hurricane states
    ...
    while diff_q > tol_q && iter_q < maxiter_q
        ...
        diff_q = maximum(abs.(q_g_new .- q_g))
        @printf("Outer iteration %d: diff_q = %.4e\n", iter_q, diff_q)
        q_g = damp_q .* q_g_new .+ (1 - damp_q) .* q_g
    ...
    global dummy_rel = copy(dummy_rel)
    @printf("Default iteration converged in %d outer iterations.\n", iter_q)

    return (b_g_vec, q_g, q_g_pf, v_guess, v_bad_guess, def_new, prob_choice, dummy_rel)
end
...
function main_country_2P_RN(country::String)
    ...
    @save "output/sim_2P_PC_RN_climate.jld2" meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim meanspread_hurr_sim medianspread_hurr_sim gdp_g_h_sim spread_g_h_sim b_g_mean i_x_sim cons_sim spread_sim def_freq_sim hur_freq_sim gdp_mean_store V_g_mean
...
@time begin
    result_2P_RN_climate = main_country_2P_RN("Jamaica")
end

# Call the plotting function using the outputs from main_country:
plt = plot_bond_price_schedule_with_gdp(result_2P_RN_climate.b_g_vec, result_2P_RN_climate.q_g, result_2P_RN_climate.gdp_vec, result_2P_RN_climate.meanDebtGDP)
display(plt)

# Saving model simulations results
V_g_2P_RN_climate = result_2P_RN_climate.V_g_mean
gamma_c = result_2P_RN_climate.gamma_c

@save "output/Vg_sim_2P_RN_climate.jld2" V_g_2P_RN_climate gamma_c