...
function default_iteration_2P_RA!(; sigma_ey, rho_y, beta, wc_par_asymm, delta, mu_r,
    ...
    alpha1 = -141     # adjust as necessary
    r_star = mu_r
    ...
    while diff_q > tol_q && iter_q < maxiter_q
        ...
        # Compute risk-adjusted default repayment expectation:
        ...
        # Optional penalty factor in high default-probability states:
        d_threshold = 0.9
        theta = 100.0
...
function main_country_2P_RA(country::String)
    N_y = 7
    N_h = 3
    N_x = N_y * N_h
    N_b_g = 50
    ...
    @save "output/sim_2P_PC_RA.jld2" meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim meanspread_hurr_sim medianspread_hurr_sim gdp_g_h_sim spread_g_h_sim b_g_mean i_x_sim cons_sim spread_sim def_freq_sim hur_freq_sim gdp_mean_store V_g_mean
...
@time begin
    result_2P_RA = main_country_2P_RA("Jamaica")
end

# Call the plotting function using the outputs from main_country:
plt = plot_bond_price_schedule_with_gdp(result_2P_RA.b_g_vec, result_2P_RA.q_g, result_2P_RA.gdp_vec, result_2P_RA.meanDebtGDP)
display(plt)

# Saving model simulations results
V_g_2P_RA = result_2P_RA.V_g_mean
gamma_c = result_2P_RA.gamma_c

@save "output/Vg_sim_2P_RA.jld2" V_g_2P_RA gamma_c