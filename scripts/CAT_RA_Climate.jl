...
function default_iteration_CAT_RA!(; sigma_ey, rho_y, beta, wc_par_asymm, delta, mu_r,
    ...
    alpha0 = 11.0             # as specified
    alpha1 = -141.0       # as specified
...
function main_country_CAT_RA(country::String)
    ...
    @save "output/sim_CAT_RA_climate.jld2" meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim meanspread_hurr_sim medianspread_hurr_sim gdp_g_h_sim spread_g_h_sim b_g_mean i_x_sim cons_sim spread_sim def_freq_sim hur_freq_sim gdp_mean_store V_g_mean
...
@time begin
    result_CAT_RA_climate = main_country_CAT_RA("Jamaica")
end

# Call the plotting function using the outputs from main_country:
plt = plot_bond_price_schedule_with_gdp(result_CAT_RA_climate.b_g_vec, result_CAT_RA_climate.q_g, result_CAT_RA_climate.gdp_vec, result_CAT_RA_climate.meanDebtGDP)
display(plt)

# Saving model simulations results
V_g_CAT_RA_climate = result_CAT_RA_climate.V_g_mean
gamma_c = result_CAT_RA_climate.gamma_c

@save "output/Vg_sim_CAT_RA_climate.jld2" V_g_CAT_RA_climate gamma_c
