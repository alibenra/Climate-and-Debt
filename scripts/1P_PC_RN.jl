...
function get_country_params_bench(country::String)
...
function main_country_1P_RN(country::String)
    ...
    country_params = get_country_params_bench(country)
    ...
    @save "output/sim_1P_PC_RN.jld2" meanBY_sim meanBY_sim_market meanspread_sim medianspread_sim stdspread_sim meanspread_hurr_sim medianspread_hurr_sim gdp_g_h_sim spread_g_h_sim b_g_mean i_x_sim cons_sim spread_sim def_freq_sim hur_freq_sim gdp_mean_store V_g_mean
...
@time begin
    result_1P_RN = main_country_1P_RN("Jamaica")
end

# Call the plotting function using the outputs from main_country:
plt = plot_bond_price_schedule_with_gdp(result_1P_RN.b_g_vec, result_1P_RN.q_g, result_1P_RN.gdp_vec, result_1P_RN.meanDebtGDP)
display(plt)

# Saving model simulations results
V_g_1P_RN = result_1P_RN.V_g_mean
gamma_c = result_1P_RN.gamma_c

@save "output/Vg_sim_1P_RN.jld2" V_g_1P_RN gamma_c