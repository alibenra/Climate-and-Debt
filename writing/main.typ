...
Economically, the truncation operator reflects the fact that even when a hurricane hits, it might not cause observable damage to economic activities, which is captured by $h_t = 1$ even when a hurricane hits with probability $pi_H$. Additionally, this formulation will also be useful in the extension to climate-contingent instruments, as it could be interpreted to reflect the uncertainty revolving around their trigger. Parametric triggers adopted in many designs of both DPCs and CAT bonds are only activated if a certain threshold is met under very specific criteria (World Bank, 2022). Therefore, even if a rare and major hurricane hits, it may not activate payouts. This conservative modeling approach preserves the log-normal shape of disaster risk while maintaining interpretability and tractability in both standard and contingent debt environments.
